...
def room_segments_to_transcript_items(room: dict[str, Any]) -> list[TranscriptItem]:
    presenter_tgt = (room.get("presenter_tgt") or DEFAULT_TARGET_LANGUAGE).strip().lower()
    ...
    for segment in room.get("segments") or []:
        ...
        if not original:
            continue
        src = (segment.get("src") or room.get("src") or DEFAULT_SOURCE_LANGUAGE).strip().lower()
    ...
    for idx, item in enumerate(items, start=1):
        ...
        if item.ts_ms:
            try:
                ...
                #     "%Y-%m-%d %H:%M:%S", time.gmtime(item.ts_ms / 1000)
                # )
            except Exception:
                timestamp = ""
        prefix = f"[{idx}]"
...
async def build_room_snapshot(
    ...
    *,
    ...
    requested_target = requested_target if requested_target in code_to_language else room_copy["presenter_tgt"]
...
async def start_room_export_package(room_id: str, payload: RoomExportRequest):
    ...
    llm_cfg = ExportLlmConfig(
        base_url=(payload.llm.base_url if payload.llm and payload.llm.base_url else room_llm.get("base_url") or DEFAULT_LLM_BASE_URL),
        api_key=(payload.llm.api_key if payload.llm and payload.llm.api_key else room_llm.get("api_key") or DEFAULT_LLM_API_KEY),
        model=(payload.llm.model if payload.llm and payload.llm.model else room_llm.get("model") or DEFAULT_LLM_MODEL),
    )
    whisper_cfg = ExportWhisperConfig(
        base_url=(payload.whisper.base_url if payload.whisper and payload.whisper.base_url else room_whisper.get("base_url") or DEFAULT_WHISPER_BASE_URL),
        api_key=(payload.whisper.api_key if payload.whisper and payload.whisper.api_key else room_whisper.get("api_key") or DEFAULT_WHISPER_API_KEY),
        model=(payload.whisper.model if payload.whisper and payload.whisper.model else room_whisper.get("model") or DEFAULT_WHISPER_MODEL),
    )

    llm_client = make_client(llm_cfg.base_url or DEFAULT_LLM_BASE_URL, llm_cfg.api_key or DEFAULT_LLM_API_KEY)
