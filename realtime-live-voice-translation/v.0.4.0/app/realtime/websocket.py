...
from app.services.persistence import persist_segment_fact_check
...
from app.services.rooms import (
    ...
    update_room_segment,
    update_room_segment_fact_check,
)
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def broadcast_fact_check_to_room(payload: dict[str, Any]) -> None:
        if not room_id:
            return
        normalized = normalize_room_id(room_id)
        async with ROOMS_LOCK:
    ...
    async def run_fact_check_for_segment(
        *,
        target_room_id: str,
        segment_id: str,
        revision: int,
        ...
        if not cfg["fact_check"].get("enabled"):
            return
        cleaned_original = (original or "").strip()
        if not cleaned_original:
            return
        try:
            fact_check = await fact_check_text(
                text=cleaned_original,
                llm_client=llm_client,
                llm_model=cfg["fact_check"].get("model") or cfg["llm"]["model"],
                tavily_api_key=cfg["fact_check"].get("tavily_api_key") or "",
                max_sources=int(cfg["fact_check"].get("max_sources") or 3),
            )
            ...
            fact_check = {
                "status": "error",
                "motivation": str(exc) or "Fact-checking failed.",
                "provider": "llm",
                "claim_text": cleaned_original,
                "checked_at": int(time.time() * 1000),
                "sources": [],
            }

        room_segment = await update_room_segment_fact_check(
            target_room_id,
            segment_id=segment_id,
            revision=revision,
            fact_check=fact_check,
        )
        if room_segment is None:
            return
        await persist_segment_fact_check(
            target_room_id,
            segment_id=segment_id,
            revision=revision,
            fact_check_json=fact_check,
        )
        await broadcast_fact_check_to_room(
            {
                "type": "fact_check",
                "segment_id": room_segment.get("segment_id") or "",
                "revision": int(room_segment.get("revision") or 0),
                "fact_check": dict(room_segment.get("fact_check") or {}),
            }
        )
        ...
        *,
        ...
        if is_final and not previous_is_final and original:
            ...
            await persist_finalized_segment(
                ...
                fact_check_json=dict(room_segment.get("fact_check") or {}),
                ts_ms=ts_ms,
            )
            track_background_task(
                asyncio.create_task(
                    run_fact_check_for_segment(
                        target_room_id=normalized_room_id,
                        segment_id=segment_id,
                        revision=segment_state["revision"],
                        original=original,
                    )
