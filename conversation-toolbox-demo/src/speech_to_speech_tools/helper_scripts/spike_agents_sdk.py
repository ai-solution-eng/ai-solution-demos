...
def _streamable_http_factory(
    ...
    """httpx2 client factory for MCPServerStreamableHttp that disables TLS
...
def _build_model(model) -> OpenAIResponsesModel:
    ...
    client = AsyncOpenAI(
        ...
        http_client=httpx2.AsyncClient(verify=False, timeout=httpx2.Timeout(300.0, connect=30.0)),
    )
...
def _fresh_client(model, timeout: float = 60.0) -> AsyncOpenAI:
    ...
    return AsyncOpenAI(
        base_url=model.base_url,
        api_key=model.api_key,
        http_client=httpx2.AsyncClient(verify=False, timeout=timeout),
    )
