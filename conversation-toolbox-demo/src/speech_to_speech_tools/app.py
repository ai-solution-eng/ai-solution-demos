...
async def test_connection(request: Request):
    """Test connectivity AND API-key validity to all three model endpoints.

    Unlike a bare reachability probe, this hits the same authenticated routes
    the real clients use (OpenAI-compatible /models for ASR/LLM and
    /v1/audio/voices for TTS), so a missing or invalid API key is surfaced
    as a failure (401/403) instead of a false positive.  Only a 2xx response
    counts as connected.
    """
    ...
    # (display name, base url, client key, authenticated probe suffix).
    # ASR/LLM base URLs are serving-host roots; the real clients append /v1
    # (BaseModel.base_url = url + "/v1"), so the probe is {base}/v1/models.
    # TTS base URL is the server root, so its probe is {base}/v1/audio/voices.
    endpoints = [
        (
            "ASR",
            body.get("asrBaseUrl", ""),
    ...
    results = {}
    async with httpx2.AsyncClient(verify=False, timeout=10.0) as client:
        for name, url, key, suffix in endpoints:
            base = (url or "").rstrip("/")
            if not base:
                results[name] = {
                    "status": "error",
                    "code": 0,
                    "message": "No base URL configured for this endpoint.",
                }
                continue
            try:
                headers = {}
                ...
                resp = await client.get(base + suffix, headers=headers, follow_redirects=False)
                ...
                results[name] = {"status": "error", "code": 0, "message": str(e)}
...
async def list_voices(
    ...
    try:
        async with httpx2.AsyncClient(verify=False, timeout=30.0) as client:
...
async def upload_voice(
    ...
    try:
        async with httpx2.AsyncClient(verify=False, timeout=120.0) as client:
