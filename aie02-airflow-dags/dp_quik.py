...
#        'weave_conn_id': Param("ce-dougdet-weaviate", type="string"),
    ...
    def cleanup_export_dir():
        ...
        if path.exists(export_dir):
            ...
            print(f"Deleted directory {export_dir}.")
            msg = 'recreated'
        else:
            print(f"{export_dir} doesn't exist so nothing to delete")
            msg = 'created'
        os.mkdir(export_dir, mode=0o775)
        print(f"{export_dir} {msg}.")
...
#                        print(f"Creating {download_dir}", flush=True)
#                        os.makedirs(download_dir, mode=0o755, exist_ok=True)
                        continue
                    print(f"Downloading {key}", flush=True)
                    s3.download_file( s3_bucket, key, path.join(download_dir, key) )
...
#                os.makedirs(download_dir, mode=0o755, exist_ok=True)
#                continue
#            print(f"Downloading {s3path}", flush=True)
#           #
#           # PERFORM Download
...
#            print(f"Downloaded {path_in_shared_volume}", flush=True)
