...
with DAG(
    ...
    def cleanup_export_dir():
        ...
        if 'AUTH_TOKEN' in os.environ:
            logging.info(f"AUTH_TOKEN={os.environ['AUTH_TOKEN']}")
        else:
            logging.info('AUTH_TOKEN not set')
        export_dir = path.join(shared_vol_base, dnld_path, dnld_dir)
        if path.exists(export_dir):
            shutil.rmtree(export_dir)
            logging.info(f"Deleted directory {export_dir}.")
            msg = 'recreated'
        else:
            logging.info(f"{export_dir} doesn't exist so nothing to delete")
            msg = 'created'
        os.mkdir(export_dir, mode=0o775)
        logging.info(f"{export_dir} {msg}.")
...
#        return filepaths
    ...
    def download_s3_file_to_shared_volume():
        ...
        for s3path in s3.list_keys(bucket_name=s3_bucket, prefix=s3_prefix):
            logging.info(f"Downloading {s3path}")
            #
            ...
            download_dir = path.join(shared_vol_base, dnld_path, dnld_dir, s3_sub_pfx)
            if not path.exists(download_dir):
                logging.info(f"Creating {download_dir}")
            ...
            logging.info(f"Downloaded {path_in_shared_volume}")
