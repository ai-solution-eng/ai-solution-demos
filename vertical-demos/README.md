...
| [Predictive Maintenance](predictive-maintenance)                        | A predictive maintenance model trained leveraging **Jupyter Notebook**, tracked in **MLFlow**, packaged with BentoML and deployed via **MLIS**.|
| [Support Ticket Assistant](support-ticket-assistant)                        | An application that can classifies tickets and provide expected resolution steps using a chat model. Also uses OCR to analyze text from network equipment photos for diagnostic purposes. Relies on **MLIS** for model deployment.|
